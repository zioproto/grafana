...
func ProvideStore(db db.DB, bus bus.Bus) *Store {
	return &Store{
		DB:  db,
		bus: bus,
		log: log.New("datasources.store"),
	}
}
...
func (s *Store) GetDataSources(ctx context.Context, query *datasources.GetDataSourcesQuery) error {
	...
	return s.DB.WithDbSession(ctx, func(dbSess *sqlstore.DBSession) error {
...
func (s *Store) GetAllDataSources(ctx context.Context, query *datasources.GetAllDataSourcesQuery) error {
	return s.DB.WithDbSession(ctx, func(sess *sqlstore.DBSession) error {
		query.Result = make([]*datasources.DataSource, 0)
...
func (s *Store) DeleteDataSource(ctx context.Context, cmd *datasources.DeleteDataSourceCommand) error {
	return s.DB.WithTransactionalDbSession(ctx, func(sess *sqlstore.DBSession) error {
		...
		err := s.getDataSource(ctx, dsQuery, sess)
		...
		if ds != nil {
			...
			if _, errDeletingPerms := sess.Exec("DELETE FROM permission WHERE scope=?",
				accesscontrol.Scope("datasources", "id", fmt.Sprint(dsQuery.Result.Id))); errDeletingPerms != nil {
...
func (s *Store) AddDataSource(ctx context.Context, cmd *datasources.AddDataSourceCommand) error {
	return s.DB.WithTransactionalDbSession(ctx, func(sess *sqlstore.DBSession) error {
		...
		// if cmd.UpdateSecretFn != nil {
		// 	if err := cmd.UpdateSecretFn(); err != nil {
		// 		s.log.Error("Failed to update datasource secrets -- rolling back update", "name", cmd.Name, "type", cmd.Type, "orgId", cmd.OrgId)
		// 		return err
		// 	}
		// }
