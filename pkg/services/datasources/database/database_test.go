...
func TestIntegrationDataAccess(t *testing.T) {
	...
	t.Run("AddDataSource", func(t *testing.T) {
		...
		t.Run("fires an event when the datasource is added", func(t *testing.T) {
			...
			store.bus.AddEventListener(func(ctx context.Context, e *events.DataSourceCreated) error {
				created = e
				return nil
			})
			...
			}, time.Duration(3)*time.Second, time.Second)
	...
	t.Run("DeleteDataSourceAccessControlPermissions", func(t *testing.T) {
		...
		errAddPermissions := store.DB.WithTransactionalDbSession(context.TODO(), func(sess *sqlstore.DBSession) error {
		...
		permCount := int64(0)
		errGetPermissions := store.DB.WithTransactionalDbSession(context.TODO(), func(sess *sqlstore.DBSession) error {
			var err error
...
func testStore(t *testing.T) *Store {
	return ProvideStore(sqlstore.InitTestDB(t), bus.ProvideBus(tracing.InitializeTracerForTest()))
}
